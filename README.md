...
The present project aims to address this gap by exploring tremor suppression through the EduExo robotic exoskeleton platform, with a focus on accessibility and ease of use for the end user. The next section provides an overview of the materials required to realise this project.
...
| Zip ties [18] | Ties the wiring together | ± 5 | €4 | [Zip ties](https://www.brico.be/nl/tuin-terras-buitenleven/moestuinen-serres/moestuin-accessoires-benodigdheden/bindmaterialen/nature-kabelbinders-nylon-15cm-25-stuks/5309665?channable=013caf696400353330393636350a&srsltid=AfmBOooQgZLrOSYoCEdjnZaxMx7UUmK5CnWu-z6v-pr-IYdbIpadAeuwli8) |
...
The potentiometer serves a dual role. When turned past approximately 55% of its travel, it triggers SUPPRESS mode; when turned below 45%, it exits SUPPRESS mode. A dead-band between these thresholds prevents oscillation near the midpoint. Within the upper half of the potentiometer range, the ADC value is mapped to a gain between 0.55 and 1.0 that controls the strength of the position-hold force applied during tremor detection. 
